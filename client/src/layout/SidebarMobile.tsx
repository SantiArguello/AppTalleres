...
const SidebarMobile = ({ ...props }) => {
	...
	return (
		<div className={`fixed inset-0 flex z-30 md:hidden ${mobileSidebarOpen ? "visible" : "invisible"}`}>
			{/* Overlay */}
			<div
				className={`overlay fixed inset-0 bg-neutral-600 bg-opacity-75 ${mobileSidebarOpen ? "visible" : "opacity-0 invisible"}`}
			...
			<div
				className={`fixed top-0 flex-1 flex flex-col w-56 bg-inherit md:hidden transition duration-300 ${
					mobileSidebarOpen ? "translate-0" : "-translate-x-full"
				} `}>
